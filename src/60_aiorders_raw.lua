...
function Ess.Raw.AIOrders.actor(g)
    local ok, drv = Ess.Safe.quiet(Vehicle.GetDriver, g)
    if ok and drv then return drv end
    return g
end

function Ess.Raw.AIOrders.goal(args)
    local ok, h = Ess.Safe.quiet(Ai.Goal, args)
    return ok and h
end

function Ess.Raw.AIOrders.haste(g, speed)
    if speed then Ess.Safe.quiet(Ai.SetHaste, g, speed) end
end
...
function Ess.Raw.AIOrders.priorityTarget(g)
    Ess.Safe.quiet(Ai.SetPriorityTarget, g)
end
...
function Ess.Raw.AIOrders.enable(g, bOn)
    Ess.Safe.quiet(Ai.Enable, g, bOn and true or false)
end
