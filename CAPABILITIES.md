...
| Namespace | What it's for | Key calls |
|---|---|---|
| `Ess.Player` | Player/character identity without the 8-getter sprawl | `.character(i)` (0 local, 1 co-op partner), `.slot(i)`, `.camera(i)`, `.pose(i)`, `.giveCash(n)`, `.giveFuel(n)`, `.targetUnderReticle(i)`, `.rumble(i, len)`, `.removeBoundaries()` |
| `Ess.Object` | Common per-object queries | `.vehicleOf(char)`, `.pollVehicleChange(char, onChange)`, `.setInvincible(guid, on, reason)`, `.distance(a, bOrX, …)`, `.heal(guid)` |
