...
| `Ess.Vehicle` | Seats/riders/entry | `.driver(veh)`, `.riders(veh)`, `.seatOf(char)`, `.enterBestSeat(char, veh)`, `.enterSeatExcluding(char, veh, excl)`, `.exit(veh, char)`, `.followGhost(template, x,y,z)`; **`Ess.Easy.Vehicle.summon(template)`** — spawn a vehicle in front + hop in the driver seat (the "I want a helicopter" → flying-it one-liner) |
